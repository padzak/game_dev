...
void BreakoutGameLevel::CreateDefaultLevel(const AARectangle& boundary)
{
	...
	float startX = static_cast<float>((int)boundary.GetWidth() - (NUM_BLOCKS_ACROSS * (BLOCK_WIDTH+1))) / 2;
	...
	{
		AARectangle blockRect = {Vec2D(startX, static_cast<float>(BLOCK_HEIGHT * (r+1))), BLOCK_WIDTH, BLOCK_HEIGHT};
...
std::vector<BreakoutGameLevel> BreakoutGameLevel::LoadLevelsFromFile(const std::string& filePath)
{
	...
	std::vector<Block> levelBlocks;
	 ...
	 layoutCommand.parseFunc = [&](ParseFuncParams params) {
		 ...
		 float startingX = 0;
		 AARectangle blockRect(Vec2D(startingX, static_cast<float>((params.lineNum+1)*BLOCK_HEIGHT)), BLOCK_WIDTH, BLOCK_HEIGHT);
