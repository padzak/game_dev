...
void BreakOut::Draw(Screen& screen)
{
	...
	Circle lifeCircle = {Vec2D(7.0f, static_cast<float>(App::Singleton().Height()) - 10.0f), 5.0f};

	for(int i = 0; i < mLives; ++i)
	{
		screen.Draw(lifeCircle, Color::Red(), true, Color::Red());
		lifeCircle.MoveBy(Vec2D(17.f, 0.f));
	}
}
...
void BreakOut::ResetGame(size_t toLevel)
{
	...
	float paddleWidth = static_cast<float>(Paddle::PADDLE_WIDTH);
	float paddleHeight = static_cast<float>(Paddle::PADDLE_HEIGHT);

	mLevels = BreakoutGameLevel::LoadLevelsFromFile(App::GetBasePath() + "assets/BreakoutLevels.txt");
	mYCutoff = screenHeight - 2*Paddle::PADDLE_HEIGHT;
	mLives = NUM_LIVES;
	mCurrentLevel = toLevel;
	AARectangle paddleRect = {Vec2D(screenWidth /2.0f - paddleWidth /2.0f, screenHeight - 3.0f * paddleHeight), Paddle::PADDLE_WIDTH, Paddle::PADDLE_HEIGHT};
	AARectangle levelBoundary = {Vec2D::Zero, static_cast<unsigned int>(screenWidth), static_cast<unsigned int>(screenHeight) };

	mLevelBoundary = {levelBoundary};

	mPaddle.Init(paddleRect, levelBoundary);
	mBall.MoveTo(Vec2D(screenWidth /2.0f, screenHeight * 0.75f));
