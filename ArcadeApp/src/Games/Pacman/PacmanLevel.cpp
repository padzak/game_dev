...
void PacmanLevel::Update(uint32_t dt, Pacman& pacman, std::vector<Ghost>& ghosts, std::vector<GhostAI>& ghostAIs)
{
	...
	for(Tile t: mTiles)
	{
		if(t.isTeleportTile)
		{
			AARectangle teleportTileAABB(t.position, t.width, mTileHeight);
...
void PacmanLevel::GetBonusItemSpriteName(std::string& spriteName, uint32_t & score) const
{
	...
	{
		if(static_cast<uint32_t>(mCurrentLevel) >= properties.begin && static_cast<uint32_t>(mCurrentLevel) <= properties.end)
		{
...
void PacmanLevel::ResetPellets()
{
	...
	uint32_t startingY = static_cast<uint32_t>(mLayoutOffset.GetY()) + PADDING + mTileHeight - 1;
	uint32_t startingX = PADDING + 3;

	const uint32_t LEVEL_HEIGHT = static_cast<uint32_t>(mLayoutOffset.GetY()) + 32 * mTileHeight;
		...
		for(uint32_t x = startingX, col = 0; x < App::Singleton().Width(); x += PADDING, ++col)
		{
			if( row == 0 || row == 22)
			{
				if(col == 0 || col == 25)
				{
					p.powerPellet = 1;
					p.score = 50;
					p.mBBox = AARectangle(Vec2D(static_cast<float>(x) - 3.0f, static_cast<float>(y) -3), mTileHeight, mTileHeight);
			...
			AARectangle rect = AARectangle(Vec2D(static_cast<float>(x), static_cast<float>(y)), PELLET_SIZE, PELLET_SIZE);
...
bool PacmanLevel::LoadLevel(const std::string& levelPath)
{
	...
	layoutCommand.parseFunc = [&layoutOffset, this](ParseFuncParams params)
		{
			float startingX = layoutOffset.GetX();
			...
			layoutOffset += Vec2D(0.0f, static_cast<float>(mTileHeight));
		};
